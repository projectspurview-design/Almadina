...
public class ConsolidatedTransactionActivity extends AppCompatActivity {
    ...
    private void goNext() {
        ...
        selectedTransactionId = sel.transBatchId;
        selectedCompanyCode   = sel.companyCode;
        selectedPrinCode      = sel.prinCode;
        selectedPickUser      = session.getUserId();

        Intent i = new Intent(this, com.example.supportapp.Pick_Consolidated.LocationDetailsActivity.class);
        i.putExtra("TRANS_BATCH_ID", selectedTransactionId);
        i.putExtra("COMPANY_CODE",   selectedCompanyCode);
        i.putExtra("PRIN_CODE",      selectedPrinCode);
        i.putExtra("PICK_USER",      selectedPickUser);
        startActivity(i);
    }
}
