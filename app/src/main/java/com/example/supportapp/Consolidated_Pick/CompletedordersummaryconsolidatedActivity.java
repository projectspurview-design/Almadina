...
// ✅ Use TOTAL_QUANTITY_SUM as total quantity
    ...
    private void calculatePerformance() {
        ...
        Log.d(TAG, "calculatePerformance active=" + active + " goal=" + goal);

        if (goal <= EPS) {
            tvPerformance.setText(active <= EPS ? "0.00%" : "N/A");
            return;
        }

        if (Math.abs(active - goal) <= EPS) {
            tvPerformance.setText("100.00%");
            return;
        }

        double performancePercent = (active / goal) * 100.0;
