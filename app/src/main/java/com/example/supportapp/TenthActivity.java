...
//    private int originalTotalLocations = 0;   // total to show at the top
    private List<Location> orderLocations = new java.util.ArrayList<>(); // list you already iterate
    ...
    // Voice moved to VoiceCommandCenter — no speech client or keycodes here
    ...
    private View mfgContainer;    // R.id.mfgContainer (row with MFG label + value)
    private View expContainer;    // R.id.expContainer (row with EXP label + value)
    private View dateDivider;     // R.id.dateDivider (the bottom line under the dates)
...
//    private ArrayList<Location> orderLocations = new ArrayList<>();
    ...
    // ===== Voice actions wiring (no phrases registered here) =====
    private final VoiceCommandCenter.Actions voiceActions = new VoiceCommandCenter.Actions() {
        @Override public void onNext() {
            // Default "Next" for this screen: confirm short or trigger scan/resume
        ...
        @Override public void onLogout() { LogoutManager.performLogout(TenthActivity.this); }

        // Tenth-specific extras
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        originalTotalLocations = getIntent().getIntExtra("TOTAL_LOCATIONS", 0);
        updateProgressDisplay();


        // Centralized voice registration (idempotent). No voice setup in this Activity.
        ...
        dateDivider   = findViewById(R.id.dateDivider); // works after you add id in XML
        ...
        if (currentOrderLocation != null) {
            ...
            // Call start job picking API after loading order details
            startJobPicking();
        } else {
            updateUIWithLocationDetails(null);
            Log.d(TAG, "No valid last picked location found for current order, fetching order details...");
            fetchOrderLocations(true);
            // Call start job picking API after fetching order details
            startJobPicking();
        }
    ...
    // --- HELPER: treat placeholder/empty strings as missing ---
    ...
    // === VOICE: all phrase registration is centralized. Only route key events here. ===
    @Override
    ...
    public void onBackPressed() {
        ...
        // Call pause job picking API before finishing
        pauseJobPicking();
    ...
    private void openPickLocationActivity() {
        ...
        if (orderLocations != null && !orderLocations.isEmpty()) {
            intent.putParcelableArrayListExtra(
                    PickLocationActivity.ORDER_LOCATIONS_LIST_KEY,
                    new ArrayList<>(orderLocations)   // <- convert List to ArrayList
            );
        }
    ...
    private void updateUIWithLocationDetails(@Nullable Location location) {
        ...
        // regular fields
        locationText.setText(
                location.getLocationCode() != null && !location.getLocationCode().isEmpty()
                        ? location.getLocationCode() : ""
        );
        quantityText.setText(
                location.getQuantity() != 0
                        ? String.valueOf(location.getQuantity()) : ""
        );
        productText.setText(
                location.getProduct() != null && !location.getProduct().isEmpty()
                        ? location.getProduct() : ""
        );
        descriptionText.setText(
        ...
        // --- DATES: format & dynamic show/hide ---
        ...
        // hide/show entire date rows so labels + values disappear together
        ...
        // divider visible only if any date is visible
        if (dateDivider != null) {
            dateDivider.setVisibility((hasMfg || hasExp) ? View.VISIBLE : View.GONE);
        }

        // --- product image loading (kept as in your version) ---
        ...
        Log.d(TAG, String.format(
                Locale.getDefault(),
                "UI updated for location: %s, Product: %s, Qty: %d, PDA Verified: %s | MFG: %s (%b), EXP: %s (%b)",
                location.getLocationCode(), location.getProduct(), location.getQuantity(),
                location.getPdAVerified(), mfg, hasMfg, exp, hasExp
        ));
    }
    /**
    ...
    private String formatDateOnly(String dateString) {
        if (isBlank(dateString)) return "";  // no data -> hide
        ...
        try {
            ...
            // already yyyy-MM-dd?
            if (dateString.matches("\\d{4}-\\d{2}-\\d{2}")) return dateString;
            return ""; // unknown format → hide
        } catch (Exception e) {
    ...
    private void handleAddDigit() {
        ...
        tvEnteredQuantity.setText(enteredQuantityBuilder.toString());
        Log.d(TAG, String.format(Locale.getDefault(), "Added digit: %d, Current number: %s", currentDigit, enteredQuantityBuilder.toString()));
    ...
    private void handleConfirmShortQuantity() {
        ...
        if (enteredQuantity > originalExpectedQuantity) {
            Toast.makeText(this, String.format(Locale.getDefault(), "Entered quantity (%d) is more than expected (%d). Proceeding anyway.", enteredQuantity, originalExpectedQuantity), Toast.LENGTH_LONG).show();
        }
        ...
        saveLastPickedLocation(currentOrderLocation);
        Toast.makeText(this, String.format(Locale.getDefault(), "Quantity set to: %d. Scan barcode to confirm pick.", newDisplayedQuantity), Toast.LENGTH_LONG).show();
        Log.d(TAG, String.format(Locale.getDefault(), "Direct quantity entered. Location quantity set to %d. Location remains unverified.", newDisplayedQuantity));
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        Log.d(TAG, String.format(Locale.getDefault(), "onActivityResult called. RequestCode: %d, ResultCode: %d", requestCode, resultCode));
        ...
        if (requestCode == MENU_REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK && data != null) {
                ...
                Log.d(TAG, String.format(Locale.getDefault(), "Received from PickLocationActivity - Selected Index: %d, Parcelable List Size: %d",
                ...
                if (!returnedLocations.isEmpty()) {
                    ...
                    if (targetLocation != null) {
                        ...
                        Log.d(TAG, String.format(Locale.getDefault(), "Current Order Location set: %s, Product: %s, Quantity: %d, PDA Verified: %s",
    ...
    private void updateUIForPauseState(boolean isPaused) {
        if (isPaused) {
            ...
            showInitialScreenState();
            updateProgressDisplay(); // ✅ refresh when resuming
    ...
    private void updateProgressDisplay() {
        ...
        int totalLocations = originalTotalLocations;
        if (totalLocations == 0 && orderLocations != null) {
            totalLocations = orderLocations.size();  // fallback
        }

        int completedLocations = 0;
        if (orderLocations != null) {
            for (Location loc : orderLocations) {
                if ("Y".equalsIgnoreCase(loc.getPdAVerified())) {
                    completedLocations++;
                }
            }
        }
        ...
        progressText.setText(completedLocations + "/" + totalLocations);
        int pct = (completedLocations * 100) / totalLocations;
    ...
    private void startJobPicking() {
        if (currentUserId == null || orderNumber == null || jobNumber == null) {
            Log.e(TAG, "Cannot start job picking - missing required data");
        ...
        if (jobAlreadyStarted) {
            Log.d(TAG, "Job " + jobNumber + " was already started previously");
            return; // Don't call API again
        }

        Log.d(TAG, "Starting job picking for: " + currentUserId + ", Order: " + orderNumber + ", Job: " + jobNumber);

        try {
            // Build URL with query parameters
            StringBuilder urlBuilder = new StringBuilder(START_JOB_PICKING_URL);
            ...
            String finalUrl = urlBuilder.toString();
            Log.d(TAG, "START_JOB_PICKING URL: " + finalUrl);

            Request request = new Request.Builder()
                    .url(finalUrl)
                    .post(RequestBody.create("", MediaType.parse("application/json"))) // Empty body for POST
                    .addHeader("accept", "*/*")
                    .addHeader("XApiKey", API_KEY)
                    .build();
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                @Override
                public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    Log.e(TAG, "Start job picking failed", e);
                    runOnUiThread(() -> {
                        Toast.makeText(TenthActivity.this, "Failed to start job: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    });
                }
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    ...
                    final String finalResponseBody = responseBody;
                    Log.d(TAG, "Start job picking response code: " + response.code());
                    Log.d(TAG, "Start job picking response body: " + finalResponseBody);

                    runOnUiThread(() -> {
                        if (response.isSuccessful()) {
                            // Mark job as started in SharedPreferences
                            markJobAsStarted(jobNumber);
                            Log.d(TAG, "Job picking started successfully: " + finalResponseBody);
                            Toast.makeText(TenthActivity.this, "Job picking started", Toast.LENGTH_SHORT).show();
                        } else {
                            Log.e(TAG, "Start job picking server error: " + response.code() + " " + finalResponseBody);
                            Toast.makeText(TenthActivity.this, "Server error starting job: " + response.code(), Toast.LENGTH_LONG).show();
                        }
                    });
                }
            });
            ...
            Log.e(TAG, "Error creating start job picking request", e);
    ...
    private void pauseJobPicking() {
        if (currentUserId == null || orderNumber == null || jobNumber == null) {
            Log.e(TAG, "Cannot pause job picking - missing required data");
            return;
        }

        Log.d(TAG, "Pausing job picking for: " + currentUserId + ", Order: " + orderNumber + ", Job: " + jobNumber);

        try {
            StringBuilder urlBuilder = new StringBuilder(PAUSE_JOB_PICKING_URL);
            urlBuilder.append("?as_login_id=").append(URLEncoder.encode(currentUserId, StandardCharsets.UTF_8.toString()));
            urlBuilder.append("&order_no=").append(URLEncoder.encode(orderNumber, StandardCharsets.UTF_8.toString()));
            urlBuilder.append("&job_no=").append(URLEncoder.encode(jobNumber, StandardCharsets.UTF_8.toString()));

            String finalUrl = urlBuilder.toString();
            Log.d(TAG, "PAUSE_JOB_PICKING URL: " + finalUrl);
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                @Override
                public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    Log.e(TAG, "Pause job picking failed", e);
                }
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    ...
                    Log.d(TAG, "Pause job picking response code: " + response.code());
                    Log.d(TAG, "Pause job picking response body: " + responseBody);

                    if (response.isSuccessful()) {
                        Log.d(TAG, "Job picking paused successfully");
                    } else {
                        Log.e(TAG, "Failed to pause job picking: " + response.code() + " " + responseBody);
                    }
                }
            });

        } catch (Exception e) {
            Log.e(TAG, "Error creating pause job picking request", e);
        }
    }

    /**
     * Resume job picking process
     */
    private void resumeJobPicking() {
        if (currentUserId == null || orderNumber == null || jobNumber == null) {
            Log.e(TAG, "Cannot resume job picking - missing required data");
            Toast.makeText(this, "Missing user or order information", Toast.LENGTH_SHORT).show();
            return;
        }

        Log.d(TAG, "Resuming job picking for: " + currentUserId + ", Order: " + orderNumber + ", Job: " + jobNumber);

        try {
            StringBuilder urlBuilder = new StringBuilder(RESUME_JOB_PICKING_URL);
            urlBuilder.append("?as_login_id=").append(URLEncoder.encode(currentUserId, StandardCharsets.UTF_8.toString()));
            urlBuilder.append("&order_no=").append(URLEncoder.encode(orderNumber, StandardCharsets.UTF_8.toString()));
            urlBuilder.append("&job_no=").append(URLEncoder.encode(jobNumber, StandardCharsets.UTF_8.toString()));

            String finalUrl = urlBuilder.toString();
            Log.d(TAG, "RESUME_JOB_PICKING URL: " + finalUrl);
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                @Override
                public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    Log.e(TAG, "Resume job picking failed", e);
                    runOnUiThread(() -> {
                        Toast.makeText(TenthActivity.this, "Failed to resume job: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    });
                }
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    ...
                    final String finalResponseBody = responseBody;
                    Log.d(TAG, "Resume job picking response code: " + response.code());
                    Log.d(TAG, "Resume job picking response body: " + finalResponseBody);

                    runOnUiThread(() -> {
                        if (response.isSuccessful()) {
                            Log.d(TAG, "Job picking resumed successfully");
                            Toast.makeText(TenthActivity.this, "Job picking resumed", Toast.LENGTH_SHORT).show();
                        } else {
                            Log.e(TAG, "Failed to resume job picking: " + response.code() + " " + finalResponseBody);
                            Toast.makeText(TenthActivity.this, "Failed to resume job: " + response.code(), Toast.LENGTH_LONG).show();
                        }
                    });
                }
            });
            ...
            Log.e(TAG, "Error creating resume job picking request", e);
    ...
    private void markJobAsStarted(String jobNo) {
        SharedPreferences prefs = getSharedPreferences("AppPrefs", MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putBoolean(JOB_STARTED_PREFIX + jobNo, true);
        editor.apply();
        Log.d(TAG, "Marked job as started: " + jobNo);
    }

    private boolean isJobStarted(String jobNo) {
        SharedPreferences prefs = getSharedPreferences("AppPrefs", MODE_PRIVATE);
        return prefs.getBoolean(JOB_STARTED_PREFIX + jobNo, false);
    }
    ...
    protected void onResume() {
        super.onResume();
        // Check if this is a resume from pause (you can add logic to determine this)
        SharedPreferences prefs = getSharedPreferences("AppPrefs", MODE_PRIVATE);
        boolean jobStarted = prefs.getBoolean(JOB_STARTED_PREFIX + jobNumber, false);

        if (jobStarted && jobNumber != null && !jobNumber.isEmpty()) {
            // Job was started before, so this might be a resume
            Log.d(TAG, "Activity resumed with previously started job, calling resume API");
            resumeJobPicking();
        }
        boolean isShortQuantityInputVisible = (shortQuantityInputContainer != null && shortQuantityInputContainer.getVisibility() == View.VISIBLE);
        ...
        Log.d(TAG, String.format(Locale.getDefault(), "onResume: ShortInputVisible=%b, ScanningFlag=%b, SkuStatusVisible=%b, PausedState=%b, CurrentLocation: %s",
