...
import com.example.supportapp.Induvidual_Pick.Location.Location;
...
public class InduvidualJobActivity extends AppCompatActivity {
    ...
    // NEW: Map to remember the original quantity from the API for each location ID
    ...
    private void updateUIWithLocationDetails(@Nullable Location location) {
        ...
        // MODIFIED: Always show quantity, including 0
    ...
    private void handleConfirmShortQuantity() {
        ...
        // --- MODIFICATION START ---
        // If the quantity field is empty, default the short quantity to 0 and proceed.
        if (enteredQuantityBuilder.length() == 0) {
            enteredShortQuantity = 0;
            Log.d(TAG, "Entered quantity was empty. Defaulting short quantity to 0.");
        } else {
            try {
        ...
        // 1. GET ORIGINAL MAX QUANTITY (From Map if exists, otherwise fallback to current)
        int originalMaxQuantity = currentOrderLocation.getQuantity(); // fallback
        ...
        // 4. CALCULATE: Original Max - Short Input = Pick Quantity (as_qty1)
        int finalPickedQuantity = originalMaxQuantity - enteredShortQuantity;

        // Update the location object with the result
        ...
        // Clear the input builder after successful confirmation
        enteredQuantityBuilder.setLength(0);
        tvEnteredQuantity.setText("0"); // Reset display to 0 for next entry
    ...
    private void findNextUnverifiedLocationAndProceed(boolean startFromNextIndex) {
        ...
        if (foundNext) {
            ...
            showInitialScreenState();
            Toast.makeText(InduvidualJobActivity.this, "Proceeding to next unverified location: " + currentOrderLocation.getLocationCode(), Toast.LENGTH_LONG).show();
            ...
            if (allLocationsVerified) {
                Toast.makeText(InduvidualJobActivity.this, "All locations picked. Finishing order.", Toast.LENGTH_LONG).show();
                ...
                Toast.makeText(InduvidualJobActivity.this, "No more unverified locations found from current point. Use 'Jump To' to select another.", Toast.LENGTH_LONG).show();
                showInitialScreenState();
    ...
    private void updateProgressDisplay() {
        ...
        Log.d(TAG, String.format(Locale.getDefault(), "Progress updated: %d/%d (%.2f%%)", completedLocations, totalLocations, (float) progressPercentage));
    }
        ...
        try {
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    ...
                    runOnUiThread(() -> {
                        Toast.makeText(InduvidualJobActivity.this, "Failed to start job: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    });
                }
        ...
        try {
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    ...
                    runOnUiThread(() -> {
                        Toast.makeText(InduvidualJobActivity.this, "Failed to resume job: " + e.getMessage(), Toast.LENGTH_LONG).show();
                    });
                }
