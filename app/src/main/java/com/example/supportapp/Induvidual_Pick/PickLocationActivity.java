package com.example.supportapp.Induvidual_Pick; // Ensure this matches your package name
...
import com.example.supportapp.Induvidual_Pick.adapter.LocationAdapter;
...
public class PickLocationActivity extends AppCompatActivity implements LocationAdapter.OnLocationClickListener {
    ...
    public static final String CURRENT_LOCATION_INDEX_KEY = "CURRENT_LOCATION_INDEX"; // New: to pass current location to PickLocationActivity
    public static final String CURRENT_LOCATION_OBJECT_KEY = "CURRENT_LOCATION_OBJECT"; // NEW: Key for passing current Location object
    ...
    // Stores the index of the location that was "current" in TenthActivity
    private int incomingCurrentLocationIndex = -1;
    // Stores the actual Location object that was "current" in TenthActivity - NEW
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupVoiceCommands();
        // Initialize OkHttpClient with SSL handling - REPLACE THE OLD INITIALIZATION
        ...
        // Get the current locations list and index passed from TenthActivity (if any)
        if (getIntent().hasExtra(ORDER_LOCATIONS_LIST_KEY)) {
            ArrayList<Location> existingLocations = getIntent().getParcelableArrayListExtra(ORDER_LOCATIONS_LIST_KEY);
            if (existingLocations != null) {
                // Filter existing locations to only show unverified ones.
                // This is important if TenthActivity sends the full list, including verified ones.
                ...
                if (!unverifiedExistingLocations.isEmpty()) {
                    // Sort them again if they weren't consistently sorted
                    Collections.sort(unverifiedExistingLocations, new Comparator<Location>() {
                        @Override
        ...
        incomingCurrentLocationObject = getIntent().getParcelableExtra(CURRENT_LOCATION_OBJECT_KEY); // NEW: Get the current Location object
        ...
        fetchLocationsFromServer(); // Always fetch fresh data to ensure it's up-to-date
    }
            ...
            httpClient.newCall(request).enqueue(new Callback() {
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                    ...
                    try (ResponseBody responseBody = response.body()) {
                        ...
                        Log.e(TAG, "Error reading response body or closing response", e);
                        responseString = "Error reading response body: " + e.getMessage();
                        runOnUiThread(() -> {
                            showLoading(false);
                            handleApiError("Failed to read server response. Details: " + e.getMessage());
                        });
                        return;
                    }
    ...
    private void parseLocationData(String jsonResponse) {
        try {
            ...
            try {
                ...
                Log.e(TAG, "JSON parsing error for response format (initial array/object check): " + e.getMessage());
                try {
                    JSONObject singleObject = new JSONObject(jsonResponse);
                    locationsArray = new JSONArray();
                    locationsArray.put(singleObject);
                    Log.d(TAG, "Attempted to parse response as a single JSON object.");
                } catch (JSONException ex) {
                    Log.e(TAG, "Failed to parse as single JSON object either.", ex);
                    runOnUiThread(() -> {
                        showLoading(false);
                        handleApiError("Invalid JSON response format. Expected array or object.\n\nRaw response: " + (jsonResponse.length() > 200 ? jsonResponse.substring(0, 200) + "..." : jsonResponse));
                    });
                    return;
                }
            }
            if (locationsArray == null || locationsArray.length() == 0) {
                runOnUiThread(() -> {
                    ...
                    handleApiError("No location data found in response.\n\nThis could mean:\n" +
                            ...
                            "• Server returned unexpected data format or an empty array.");
                });
                return;
            }
            ...
            for (int i = 0; i < locationsArray.length(); i++) {
                try {
                    ...
                    if (location.getPdAVerified().equalsIgnoreCase("N")) {
                        ...
                        Log.d(TAG, "Parsed and added unverified location: " + location.toString());
                    } else {
                        Log.d(TAG, "Skipping verified location: " + location.getLocationCode() + " (pdA_VERIFIED: " + location.getPdAVerified() + ")");
                    }
                } catch (JSONException e) {
                    Log.w(TAG, "Error parsing individual location object at index " + i + ", skipping.", e);
                }
            }
            runOnUiThread(() -> {
                showLoading(false);
                if (fetchedLocations.isEmpty()) {
                    handleApiError("No valid unverified location data could be parsed or found from the response.\n\n" +
                            "This might be due to unexpected data structure, missing essential fields, or all locations are already verified.");
                } else {
                    Collections.sort(fetchedLocations, new Comparator<Location>() {
                        @Override
                        public int compare(Location l1, Location l2) {
                            int locCompare = l1.getLocationCode().compareTo(l2.getLocationCode());
                            if (locCompare == 0) {
                    ...
                    // 1. Try to select the incoming current location object if it's in the fetched list and unverified
                    if (incomingCurrentLocationObject != null) {
                        for (int i = 0; i < locationList.size(); i++) {
                            if (locationList.get(i).getId().equals(incomingCurrentLocationObject.getId()) &&
                                    locationList.get(i).getPdAVerified().equalsIgnoreCase("N")) {
                                selectionIndex = i;
                                Log.d(TAG, "Auto-selected incoming current location: " + incomingCurrentLocationObject.getLocationCode() + " at index " + selectionIndex);
                                break;
                            }
                        }
                    }
                    // 2. If not found or not unverified, try to use the incoming current location index (if valid and unverified)
                    if (selectionIndex == -1 && incomingCurrentLocationIndex != -1 && incomingCurrentLocationIndex < locationList.size()) {
                        if (locationList.get(incomingCurrentLocationIndex).getPdAVerified().equalsIgnoreCase("N")) {
                            selectionIndex = incomingCurrentLocationIndex;
                            Log.d(TAG, "Auto-selected incoming current location index: " + selectionIndex);
                        }
                    }
                    // 3. Fallback: If no previous current location or it's verified/missing, select the first unverified
                    if (selectionIndex == -1 && !locationList.isEmpty()) {
                        for (int i = 0; i < locationList.size(); i++) {
                            if (locationList.get(i).getPdAVerified().equalsIgnoreCase("N")) {
                                selectionIndex = i;
                                Log.d(TAG, "Auto-selected first unverified location: " + locationList.get(i).getLocationCode() + " at index " + selectionIndex);
                                break;
                            }
                        }
                    }
                    ...
                    if (selectionIndex != -1) {
                        ...
                        // If no unverified locations found at all in the fetched list
                        locationAdapter.setSelectedPosition(RecyclerView.NO_POSITION);
                        Toast.makeText(PickLocationActivity.this, "All locations for this order are already verified.", Toast.LENGTH_LONG).show();
                    }
                }
            });
        } catch (Exception e) {
            Log.e(TAG, "Unexpected error during parsing or initial JSON handling", e);
            runOnUiThread(() -> {
                showLoading(false);
    ...
    private void updateLocationList(ArrayList<Location> newLocations) {
        ...
        locationAdapter.notifyDataSetChanged();
        // The auto-selection logic is now handled in parseLocationData after the list is updated
        updateArrowVisibility();
        String message = "Successfully loaded " + newLocations.size() + " unverified location" +
    ...
    private void handleApiError(String errorMessage) {
        ...
        btnSelectLocation.setOnClickListener(v -> {
            btnSelectLocation.setText("SELECT");
            setupNavigationButtons(); // Re-setup original listeners
    ...
    private void navigateListByOneStep(int direction) {
        ...
        if (currentSelectedPos == RecyclerView.NO_POSITION && !locationList.isEmpty()) {
            currentSelectedPos = (direction > 0) ? -1 : 0; // If nothing selected, start from before first or at first
        }
    ...
    private void setupNavigationButtons() {
        ...
        btnSelectLocation.setOnClickListener(v -> {
            ...
            if (selectedLocation != null) {
                ...
                // Pass the full *filtered and sorted* list back to TenthActivity
