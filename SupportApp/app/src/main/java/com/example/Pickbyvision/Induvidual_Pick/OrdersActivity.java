...
public class OrdersActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Clear any paused status
        prefs.edit().remove("OrdersActivityPaused").apply();
    ...
    private void loadPicklistJobData(String loginId) {
        executorService.execute(() -> {
            try {
                ...
                Response response = UnsafeOkHttpClient.get().newCall(request).execute();

                if (response.isSuccessful()) {
                    String responseBody = response.body().string();
                    parsePicklistResponse(responseBody);
                } else {
                    showFallback();
                }
            } catch (Exception e) {
